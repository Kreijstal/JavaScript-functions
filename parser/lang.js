var parse = require('./parser-constants.js')
/*
...
var expressionFeatures = {
    ...
    array: function sequence(match) {
      ...
      match.iterator |= 0;
      //get next iteration
      var val = match.pattern[match.iterator++];
      ...
      /**
          ...
          result
          context */
      string: function literalmatch(match, args) {
        ...
        var textToParse=args.textToParse
        var str = textToParse.substr(match.indexOf - (match.reverse ? match.pattern.length : 0), match.pattern.length),
          //trim string to match, to be as long as str is (from match.indexOf-parseContext.indexOf), let that be str2
          str2 = match.pattern.substr(match.indexOf - match.startIndexOf, str.length)
        ...
        if (str === str2) {
          ...
          if (match.pattern.length > str.length) {
            if (args.isFinal) return [parse.THROW, "String match: EOF Reached"]
            return [parse.HALT]
          } else {
            //else parse.STEP_OUT
        ...
        //else parse.THROW
          ...
          //debugger
          return [parse.THROW, "String Error: expected \"" + match.pattern + "\", found \"" + str + "\" at index:" + match.indexOf]
        }
      },
      meta: {
        restorable: ["repetition", "or"]
      },
      object: {
        /**
    Uses following properties
    ...
    childStep */
        repetition: function quantifier(match) {
          ...
          if (match.restored) {
            ...
            switch (match.pattern.quantifier) {
              case "greedy":
                //attempt to take 1 less unit, if there is less than from, repetition fails
                if (match.matches.length > match.pattern.from) {
                  match.matches.pop();
                  //match.result.pop();
              ...
              case "lazy":
                ...
                if (match.matches.length < match.pattern.to) {
                  match.lazypass = true;
          ...
          //if children returned result
            ...
            if (match.childStep.fail || (match.matches.length === match.pattern.to)) { //could be restored, or could be group value was falsy
              ...
              if ((match.pattern.quantifier !== "lazy") && (match.pattern.from <= match.matches.length && match.pattern.to >= match.matches.length)) {
          ...
          if ((match.matches.length > match.pattern.from) && (match.pattern.quantifier === "lazy") && (!match.lazypass)) {
            return [parse.STEP_OUT]
          }
          match.lazypass = false;
          return [parse.STEP_IN, match.pattern.child];
        },
        pointer: function(match, args) {
          if (match.childStep) {
            if (match.childStep.fail)
          ...
          return [parse.STEP_IN, args.grammar[match.pattern.value]];
        },
        or: function alternation(match) { //well shit
          //{type:"or",child:node[]}
          if (match.childStep) {
            if ((match.childStep.fail || match.restored) && (match.pattern.choices.length === match.iterator)) { //none of the choices work
          ...
          match.iterator |= 0;
          var v = match.pattern.choices[match.iterator++];
          if (v) {
        ...
        wildcard: function wildcard(match, args) {
          //{type:"wildcard",value:[{from:5,to:7},"a","b"],negative:false}
          var text=args.textToParse
          var v,t = text[match.indexOf - match.reverse];
          if (!t && args.isFinal) return [parse.THROW, "Wildcard can't match empty string"];
          v = match.pattern.value.find(function(a) {
            if (typeof a === "object") {
              return a.to >= t.charCodeAt() && t.charCodeAt() >= a.from
            } else {
              return a === t
            }
          })
          v = match.pattern.negative ? (v ? false : t) : v;
          if (t === "") {
            return [parse.HALT];
          }
          if (v) {
        ...
        reverse: function(match) {
          ...
          return [match.STEP_IN, match.pattern.value]
        },
        variable: function(match,args) {
          var variables = args.parseContext.variables;
          if (match.childStep) {
            //I think most steps return a result and an indexOf to parent
            if (match.childStep.fail) return [parse.THROW, match.childStep.failMsg]
            match.result = variables[match.pattern.key] = match.childStep.result;
            match.indexOf = match.childStep.indexOf
            return [parse.STEP_OUT]
          }
          if (typeof match.pattern.value == "undefined") {
            match.result = variables[match.pattern.key]
            if (!match.result) {
              throw new ReferenceError("Value is undefined, and there is no variableKey \"" + match.pattern.key + "\" set.")
            }
            return [parse.STEP_IN, match.result]
          } else {
            return [parse.STEP_IN, match.pattern.value]
          }
        },
      ...
      null: function() {
        return [parse.THROW, "attempted to match null"]
      }
    }
